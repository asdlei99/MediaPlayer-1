...
if (std::string(filename).compare(0, 5, "/dev/")==0)
{
    ...
    if (iformat)
    {
        av_log(NULL, AV_LOG_INFO, "Found input format: %s", format.c_str());
    }
    else
    {
...
{
    av_log(NULL, AV_LOG_INFO, "Cannot open file %s for audio", filename);
...
AVFrame *       frame = JRFFMPEG_ALLOC_FRAME();
if (!frame)
    return -1;
...
#else // JRFFMPEG_ABLE_PLANAR_AUDIO
    ...
    do
    {
        ...
        if(readPacketRez < 0)
        {
                ...
                m_fmt_ctx_ptr->pb->eof_reached)
            {
                ...
                //ros: "av_read_frame() returned " << AvStrError(readPacketRez) << std::endl;
...
const double            start_timer_micros      = now_micros();
//
...
while (true)
{
    ...
    //
    const double    ellapsed_time_micros = now_micros() - start_timer_micros;
